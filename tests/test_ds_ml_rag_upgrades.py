...
def test_multi_objective_reward_with_dte_friction():
    ...
    res_7dte = compute_trade_reward(
        pnl=100.0, credit=1.0, max_loss=400.0, dte_at_exit=7, dte_at_entry=30
    )
    assert res_7dte["components"]["dte_friction_penalty"] == 0.0

    # Exit at 0 DTE -> max friction penalty (0.5)
    res_0dte = compute_trade_reward(
        pnl=100.0, credit=1.0, max_loss=400.0, dte_at_exit=0, dte_at_entry=30
    )
...
def test_ml_trade_gate_evaluation():
    ...
    res = evaluate_ml_trade_gate(
        strategy="spy_put_credit", ticker="SPY", regime="calm", min_confidence=0.70
    )
...
def test_regime_aware_rag_query():
    ...
    results = rag.query_lessons_with_regime(
        "options risk management", regime_context=regime_context, top_k=3
    )
