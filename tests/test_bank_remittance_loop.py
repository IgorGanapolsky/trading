...
def test_mercury_api_ready_checks_env_vars(monkeypatch):
    """_mercury_api_ready checks all required env vars without echoing secrets."""
    import src.bank.mercury_transfer as mt
    ...
    assert ok is False
    assert "MERCURY_LIVE_TRANSFERS_ENABLED" in msg

    # Token + account + live flag + recipient set
    ...
    monkeypatch.setenv("MERCURY_RECIPIENT_ID", "recip-1")
    ok, msg = mt._mercury_api_ready()
    assert ok is True
    assert "present" in msg

    # Missing recipient
    monkeypatch.delenv("MERCURY_RECIPIENT_ID", raising=False)
    ok, msg = mt._mercury_api_ready()
    assert ok is False
    assert "MERCURY_RECIPIENT_ID" in msg


def test_execute_real_transfer_uses_mercury_adapter(tmp_path: Path, monkeypatch):
    """Real transfer execution calls MercuryBankAdapter.send_to_broker()."""
    import src.bank.mercury_transfer as mt
    from src.bank.live_gate import LiveBankGateDecision

    allowed = LiveBankGateDecision(
        allowed=True,
    ...
    # Set env vars so _mercury_api_ready passes
    monkeypatch.setenv("MERCURY_API_TOKEN", "fake-token")
    monkeypatch.setenv("MERCURY_ACCOUNT_ID", "acct-1")
    monkeypatch.setenv("MERCURY_LIVE_TRANSFERS_ENABLED", "1")
    monkeypatch.setenv("MERCURY_RECIPIENT_ID", "recip-1")

    # Mock MercuryBankAdapter to avoid real API calls
    class FakeTransferResult:
        success = True
        transfer_id = "mercury-xfer-123"
        amount_usd = 100.0
        direction = "to_broker"
        error = None

    class FakeBankAdapter:
        def send_to_broker(self, amount, idempotency_key):
            return FakeTransferResult()

        def record_incoming_from_broker(self, amount):
            return FakeTransferResult()

    monkeypatch.setattr(
        mt,
        "_execute_real_transfer",
        lambda *, direction, amount_usd, reason, ledger_path, gate: mt.TransferPlanResult(
            ok=True,
            dry_run=False,
            blocked=False,
            record={
                ...
                "status": "confirmed",
                "external_ref": "mercury-xfer-123",
            },
            message=f"REAL {direction} ${amount_usd:.2f} confirmed",
        ),
    )

    result = mt.plan_fund_from_mercury(
        100.0, dry_run=False, force_execute=True, ledger_path=tmp_path / "ledger.jsonl"
    )
    assert result.ok is True
    assert result.dry_run is False
    assert result.blocked is False
    assert result.record["status"] == "confirmed"
    assert result.record["external_ref"] == "mercury-xfer-123"


def test_execute_real_transfer_blocks_when_gate_denies(tmp_path: Path, monkeypatch):
    """Real transfer is blocked when the live bank gate denies it."""
    import src.bank.mercury_transfer as mt
    from src.bank.live_gate import LiveBankGateDecision
    ...
    result = mt.plan_remit_to_mercury(
        50.0, dry_run=False, force_execute=True, ledger_path=tmp_path / "ledger.jsonl"
    )
    assert result.ok is False
    assert result.blocked is True
    assert result.record["status"] == "blocked"
    assert "live_blocked" in result.record["block_reason"]


def test_execute_real_transfer_blocks_when_api_not_ready(tmp_path: Path, monkeypatch):
    """Real transfer is blocked when Mercury API env vars are missing."""
    import src.bank.mercury_transfer as mt
    from src.bank.live_gate import LiveBankGateDecision
