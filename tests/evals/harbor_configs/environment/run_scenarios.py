...
class GoldenScenarioRunner:
    ...
    def run_all(self, verbose: bool = True) -> dict:
        ...
        total_evals = sum(1 for r in report["all_eval_results"] for e in r["eval_results"]) or max(
            1, len(report["scenarios_passed"]) + report["scenarios_failed"]
        )

        # Calculate overall pass rate as percentage of successful evals
        successful_evals = sum(
            1 for s in self.scenarios for r in self.evals.run_all_evals(s["proposal"])
        )
        ...
        print(
            f"Scenario pass rate: {100 * report['scenarios_passed'] / report['scenarios_completed']:.0f}%"
        )
...
def main():
    ...
    parser.add_argument("--all", action="store_true", default=True, help="Run all golden scenarios")
    parser.add_argument("--scenario", type=str, default=None, help="Run specific scenario by ID")
    parser.add_argument(
        "--verbose", action="store_true", default=True, help="Print detailed results"
    )
