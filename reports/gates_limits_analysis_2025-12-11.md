...
| 0.5 | Bull/Bear Debate | ENABLED | Multi-perspective analysis (42% CAGR improvement claim) |
...
- RL Threshold: `RL_CONFIDENCE_THRESHOLD=0.45` (line 746, reduced from 0.6)
- Sentiment Threshold: `LLM_NEGATIVE_SENTIMENT_THRESHOLD=-0.2` (line 1101)
...
### Priority 1: Widen Exit Thresholds (CRITICAL)
...
### Priority 2: Enable Gate 2 Selectively

Since Gates 2/3 are disabled, consider **enabling Gate 2 only** with relaxed threshold:

```bash
# .env changes to test
RL_FILTER_ENABLED=true
RL_CONFIDENCE_THRESHOLD=0.40  # Was 0.45, further relaxed
LLM_SENTIMENT_ENABLED=false   # Keep disabled to isolate RL impact
```

### Priority 3: VIX-Adaptive Position Sizing

Current sizing ignores volatility. Add VIX-based allocation:

```python
# src/core/risk_manager.py - suggested enhancement
def calculate_vix_adjusted_size(self, base_size: float, vix: float) -> float:
...
### Priority 4: Threshold Sensitivity Sweep

Add to backtest matrix (`scripts/run_backtest_matrix.py`):

```python
SENSITIVITY_SWEEP = {
    "rl_threshold": [0.35, 0.40, 0.45, 0.50, 0.55],
    "sentiment_threshold": [-0.3, -0.2, -0.1, 0.0],
...
*Analysis timestamp: 2025-12-11T00:00:00Z*
