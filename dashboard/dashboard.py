...
class DashboardDataManager:
    ...
    def _generate_sample_positions(self) -> pd.DataFrame:
        ...
        positions = [
            {'symbol': 'AAPL', 'quantity': 15.5, 'avg_entry_price': 175.20, 'current_price': 182.30,
             'market_value': 2825.65, 'cost_basis': 2715.60, 'unrealized_pnl': 110.05, 'unrealized_pnl_pct': 4.05},
            {'symbol': 'GOOGL', 'quantity': 8.0, 'avg_entry_price': 138.50, 'current_price': 142.10,
             'market_value': 1136.80, 'cost_basis': 1108.00, 'unrealized_pnl': 28.80, 'unrealized_pnl_pct': 2.60},
            {'symbol': 'MSFT', 'quantity': 12.0, 'avg_entry_price': 375.80, 'current_price': 389.20,
             'market_value': 4670.40, 'cost_basis': 4509.60, 'unrealized_pnl': 160.80, 'unrealized_pnl_pct': 3.57},
            {'symbol': 'TSLA', 'quantity': 5.0, 'avg_entry_price': 242.00, 'current_price': 238.50,
             'market_value': 1192.50, 'cost_basis': 1210.00, 'unrealized_pnl': -17.50, 'unrealized_pnl_pct': -1.45},
            {'symbol': 'NVDA', 'quantity': 10.0, 'avg_entry_price': 485.00, 'current_price': 512.30,
             'market_value': 5123.00, 'cost_basis': 4850.00, 'unrealized_pnl': 273.00, 'unrealized_pnl_pct': 5.63}
        ]
...
def render_daily_pnl_chart(performance: Dict):
    ...
    if pnl_data:
        ...
        fig.add_trace(go.Bar(
            x=df['date'],
            y=df['pnl'],
            marker_color=df['pnl'].apply(lambda x: '#28a745' if x >= 0 else '#dc3545'),
            name='Daily P/L'
        ))
...
def render_recent_trades(trades_df: pd.DataFrame, limit: int = 10):
    ...
    if not trades_df.empty:
        ...
        display_df = recent_trades[['timestamp', 'symbol', 'side', 'quantity', 'price', 'amount', 'strategy', 'pnl', 'status']].copy()
