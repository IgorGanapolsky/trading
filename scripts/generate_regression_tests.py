...
'''
        ...
        # Group by severity
        critical = [lesson for lesson in lessons if lesson.get("severity") == "CRITICAL"]
        high = [lesson for lesson in lessons if lesson.get("severity") == "HIGH"]
        other = [lesson for lesson in lessons if lesson.get("severity") not in ["CRITICAL", "HIGH"]]
...
def main():
    ...
    if args.lesson:
        lessons = [lesson for lesson in lessons if lesson.get("id", "").lower() == args.lesson.lower()]
        if not lessons:
    ...
    print(f"Critical:               {len([lesson for lesson in lessons if lesson.get('severity') == 'CRITICAL'])}")
    print(f"High:                   {len([lesson for lesson in lessons if lesson.get('severity') == 'HIGH'])}")
