...
def generate_daily_summary_post(date_str: str, lessons: list[dict]) -> str:
    ...
    critical_lessons = [lsn for lsn in lessons if lsn["severity"] == "CRITICAL"]
    if critical_lessons:
        lessons_md += "\n## The Hard Lessons\n\n"
    ...
    high_lessons = [lsn for lsn in lessons if lsn["severity"] == "HIGH"]
    if high_lessons:
    ...
    other_lessons = [lsn for lsn in lessons if lsn["severity"] in ["MEDIUM", "LOW"]]
    if other_lessons:
