...
def main():
    ...
    dates_to_check = [
        (today - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(1, days_to_check + 1)
    ]
    ...
    if len(skip_days) >= days_to_check:
        ...
        print(
            f"\n✅ No consecutive skips detected ({len(skip_days)}/{days_to_check} days had skips)"
        )
        sys.exit(0)
