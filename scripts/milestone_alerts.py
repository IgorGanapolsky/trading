...
def check_milestone_criteria(milestone_day: int, state: dict) -> dict:
    ...
    if milestone_day == 30:
        criteria = [
            {
                "name": "Total Trades ≥30",
                "met": total_trades >= 30,
                "current": total_trades,
                "required": 30,
            },
            {
                "name": "Options Win Rate ≥55%",
                "met": options["win_rate"] >= 55,
                "current": f"{options['win_rate']:.1f}%",
                "required": "55%",
            },
            {
                "name": "Positive P/L",
                "met": total_pl > 0,
                "current": f"${total_pl:.2f}",
                "required": ">$0",
            },
        ]
        ...
        criteria = [
            {
                "name": "Total Trades ≥60",
                "met": total_trades >= 60,
                "current": total_trades,
                "required": 60,
            },
            {
                "name": "Options Win Rate ≥60%",
                "met": options["win_rate"] >= 60,
                "current": f"{options['win_rate']:.1f}%",
                "required": "60%",
            },
            {
                "name": "Total Profit ≥$200",
                "met": total_pl >= 200,
                "current": f"${total_pl:.2f}",
                "required": "$200",
            },
        ...
        criteria = [
            {
                "name": "Total Trades ≥100",
                "met": total_trades >= 100,
                "current": total_trades,
                "required": 100,
            },
            {
            ...
            {
                "name": "Options Trades ≥50",
                "met": options["total"] >= 50,
                "current": options["total"],
                "required": 50,
            },
            {
                "name": "Total Profit ≥$500",
                "met": total_pl >= 500,
                "current": f"${total_pl:.2f}",
                "required": "$500",
            },
        ...
        milestones.append(
            {
                ...
                "progress_pct": sum(1 for c in criteria_check["criteria"] if c["met"])
                / len(criteria_check["criteria"])
                * 100
...
def get_next_actions(milestones: list[dict], state: dict) -> list[str]:
    ...
    for m in milestones:
        if not m["ready"]:
            ...
            for c in failing[:3]:  # Top 3 priorities
                actions.append(
                    f"Work on: {c['name']} (currently: {c['current']}, need: {c['required']})"
                )
            break
    ...
    if current_day < 30:
        actions.append(
            f"Execute {max(1, (30 - state.get('performance', {}).get('total_trades', 0)) // (30 - current_day))} trades per day to reach 30 by Day 30"
        )
...
def print_report(state: dict, summary: bool = False, upcoming: bool = False):
    ...
    if summary:
        next_milestone = next(
            (m for m in milestones if m["status"] not in ["REACHED"]), milestones[-1]
        )
        print(
            f"📅 Day {current_day}/90 | Next: {next_milestone['name']} in {next_milestone['days_until']}d | Ready: {'✅' if next_milestone['ready'] else '❌'} ({next_milestone['progress_pct']:.0f}%)"
        )
        return
    ...
    print(
        f"   Current Day: {current_day}/90 | Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    print("=" * 70)
    ...
    for m in milestones:
        ...
        print(
            f"   Status: {m['status']} | Days Until: {m['days_until']} | Ready: {'✅' if m['ready'] else '❌'}"
        )
        print(
            f"   Progress: [{'█' * int(m['progress_pct'] / 10)}{'░' * (10 - int(m['progress_pct'] / 10))}] {m['progress_pct']:.0f}%"
        )
    ...
    try:
        ...
        datetime.now() - timedelta(days=current_day)
