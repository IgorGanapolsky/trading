...
class DynamicInvestmentScaler:
    """
    Dynamic daily investment scaling based on floating P/L.

    Formula: daily_invest = min(10 + 0.3 * floating_pnl, 50)
    - Base: $10/day
...
def run_all_automation():
    ...
    # 3. Daily investment scaling
    print("\n--- Daily Investment Calculation ---")
    scaler = DynamicInvestmentScaler(paper=True)
    investment_result = scaler.calculate_daily_investment()
    print(f"Today's Investment: ${investment_result['daily_amount']:.2f}")
    print(f"Floating P/L: ${investment_result.get('floating_pnl', 0):.2f}")
    print(f"Formula: {investment_result.get('formula', 'N/A')}")
    ...
    return {
        ...
        "daily_investment": investment_result,
    }
