...
def get_python_files(root: Path, ignore_dirs: list[str] = None) -> list[Path]:
...
def build_import_graph(files: list[Path], project_root: Path) -> dict[str, set[str]]:
...
def find_isolated_module_groups(
    ...
    for prefix in prefixes:
        ...
        for module in group_modules:
            importers = import_graph.get(module, set())
            external_importers.update(imp for imp in importers if not imp.startswith(prefix + "."))
...
def main():
    parser = argparse.ArgumentParser(description="Detect dead code and unused modules")
    ...
    parser.add_argument(
        "--threshold-kb", type=int, default=10, help="Minimum size in KB to report unused modules"
    )
    ...
    significant_unused = [m for m in unused_modules if m["size_bytes"] > threshold_bytes]
    ...
    all_modules = {
        str(f.relative_to(PROJECT_ROOT).with_suffix("")).replace("/", ".") for f in files
    }
    ...
    if args.report == "json":
        ...
        if isolated_groups:
            ...
            for group in isolated_groups:
                print(
                    f"   {group['prefix']}/ ({group['module_count']} modules, "
