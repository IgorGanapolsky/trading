...
class DashboardGenerator:
    ...
    def load_data(self) -> dict:
        ...
        data = {"performance_log": [], "system_state": {}, "latest_snapshot": None}
    ...
    def generate_category_pl_bar_chart(self, snapshot: dict) -> str:
        ...
        bars1 = ax.bar(
            [i - width / 2 for i in x], realized, width, label="Realized P/L", color="#4ECDC4"
        )
        bars2 = ax.bar(
            [i + width / 2 for i in x], unrealized, width, label="Unrealized P/L", color="#FF6B6B"
        )
    ...
    def generate_markdown_dashboard(self, data: dict) -> str:
        ...
        md.append(
            f"**Total P/L:** ${snapshot.get('total_pl', 0):+,.2f} ({snapshot.get('total_pl_pct', 0):+.2f}%)"
        )
        ...
        if weekly:
            ...
            for cat in ["Crypto", "Equities", "Options", "Bonds"]:
                ...
                pl_emoji = (
                    "🟢" if cat_data["total_pl"] > 0 else "🔴" if cat_data["total_pl"] < 0 else "⚪"
                )
                md.append(
                    f"| {pl_emoji} **{cat}** | ${cat_data['total_pl']:+,.2f} | {cat_data['trades']} | {cat_data['win_rate']:.1f}% |"
                )
            md.append("")
        ...
        try:
            ...
            if recent_alerts:
                ...
                for alert in recent_alerts:
                    severity_emoji = {"info": "🎉", "warning": "⚠️", "critical": "🚨"}.get(
                        alert["severity"], "📢"
                    )
        ...
        md.append(
            f"**Challenge Day:** {challenge_info.get('current_day', 'N/A')} / {challenge_info.get('total_days', 90)}"
        )
        ...
        md.append(
            f"**Total Account P/L:** ${account_info.get('total_pl', 0):+,.2f} ({account_info.get('total_pl_pct', 0):+.2f}%)"
        )
