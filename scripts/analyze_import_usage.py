...
class ImportUsageTracker:
    ...
    def save_snapshot(self):
        ...
        serializable_snapshot = {
            module: list(importers) for module, importers in self.current_snapshot.items()
        }
        ...
        self.history = [s for s in self.history if datetime.fromisoformat(s["timestamp"]) > cutoff]
    ...
    def analyze_trends(self, module: str, days: int = 30) -> dict:
        ...
        recent_snapshots = [
            s for s in self.history if datetime.fromisoformat(s["timestamp"]) > cutoff
        ]
    ...
    def detect_anomalies(self) -> list[dict]:
        ...
        for module, importers in self.current_snapshot.items():
            ...
            if importers:
                ...
                external_importers = [
                    imp for imp in importers if not imp.startswith(module_package)
                ]
...
def main():
    parser = argparse.ArgumentParser(description="Analyze import usage patterns with ML learning")
    parser.add_argument("--learn", action="store_true", help="Save current snapshot for learning")
    parser.add_argument("--trend", type=int, metavar="DAYS", help="Analyze trends over N days")
    parser.add_argument("--module", type=str, help="Analyze specific module trend")
    parser.add_argument(
        "--generate-lessons", action="store_true", help="Auto-generate lessons from anomalies"
    )
