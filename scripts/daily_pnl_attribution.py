...
def calculate_pnl_attribution(trades: list[dict], state: dict) -> dict:
    ...
    attribution = defaultdict(
        lambda: {"realized_pl": 0.0, "trades": 0, "wins": 0, "losses": 0, "symbols": set()}
    )
...
def generate_report(attribution: dict) -> str:
    ...
    sorted_cats = sorted(attribution.items(), key=lambda x: x[1]["realized_pl"], reverse=True)
    ...
    if sorted_cats:
        ...
        for cat, data in losers:
            if data["trades"] >= 3:  # Only flag if enough data
                lines.append(
                    f"❌ {cat} is LOSING money (${data['realized_pl']:.2f}) - REVIEW STRATEGY"
                )
...
def main():
    ...
    if args.json:
        ...
        # Save report
        report_file = (
            PROJECT_ROOT / "reports" / f"pnl_attribution_{datetime.now().strftime('%Y%m%d')}.md"
        )
