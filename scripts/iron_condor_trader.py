...
class IronCondorStrategy:
    ...
    def get_underlying_price(self) -> float:
        ...
        # SPY was actually ~$692, causing PUT-only fills (CALL strikes too low)
        try:
        ...
        # SPY was ~$692 on Jan 16, 2026 per trade history
        fallback_price = 692.0
                ...
                if api_key and secret:
                        ...
                        try:
                            ...
                            if symbol in quotes:
                                ...
                                if side == OrderSide.BUY:
                                    price = ask if ask > 0 else 0.50
                                else:
                                    price = bid if bid > 0 else 0.50
                                logger.info(f"   {symbol}: bid=${bid:.2f}, ask=${ask:.2f}, using ${price:.2f}")
                                return price
                        except Exception as price_err:
                            logger.warning(f"   Could not get price for {symbol}: {price_err}")
                        # Fallback to reasonable estimate based on delta
                        return 1.50 if "C" in symbol else 1.00
                    ...
                    for sym, side, leg_name in legs:
                        try:
                            ...
                            order_ids.append({"leg": leg_name, "order_id": str(order.id), "price": limit_price})
                            logger.info(f"   ✅ {leg_name} order submitted: {order.id} @ ${limit_price:.2f}")
