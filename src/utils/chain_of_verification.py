...
class ChainOfVerification:
    ...
    def _gather_evidence(self, source: str) -> dict[str, Any]:
        ...
        if source.startswith("file:"):
            ...
            cmd = source[4:].strip()
            # SECURITY FIX Feb 17, 2026 (P0 tech debt - shell injection):
            # Exact command whitelist with pre-built argv lists.
            # shell=False prevents injection even if prefix-matching is bypassed.
            ...
            for key, argv in ALLOWED_COMMANDS.items():
                if cmd == key:
                    matched_argv = argv
                    break
            # Allow "ls <path>" with a single safe argument (no shell metacharacters)
            if matched_argv is None and cmd.startswith("ls "):
                ls_arg = cmd[3:].strip()
                if ls_arg and not any(c in ls_arg for c in ";|&$`(){}[]<>!\\'\"\n\t"):
                    matched_argv = ["ls", ls_arg]
            ...
            try:
                result = subprocess.run(  # noqa: S603
                    matched_argv, capture_output=True, text=True, timeout=10
                )
