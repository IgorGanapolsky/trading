...
def parse_pdf(path: Path) -> ParsedDocument:
    ...
    for name, fn in (
        ...
        if quality.passed or (text and not quality.garbage and not quality.likely_scanned):
        ...
        # keep last non-empty attempt if later backends fail
        if text:
            warnings.append(f"{name}_quality_fail:{','.join(quality.reasons)}")
            # if only quality fails due to scan, still return with failed quality
            if quality.likely_scanned or quality.empty_extract:
                md = text + _tables_to_markdown_appendix(tables)
                return ParsedDocument(
                    source_path=str(path),
                    format="pdf",
                    ...
                    metadata={**meta, "attempts": attempts},
                    content_hash=_content_hash(text),
                    warnings=warnings + (["REQUIRES_OCR"] if quality.likely_scanned else []),
                )
