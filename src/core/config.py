...
OPTIMIZED_AMOUNTS = {
    tier: OPTIMIZED_DAILY_INVESTMENT * pct
    for tier, pct in OPTIMIZED_ALLOCATION.items()
}
...
class AppConfig(BaseSettings):
    ...
    # Order Execution
    USE_LIMIT_ORDERS: bool = Field(default=True, description="Use limit orders instead of market orders to reduce slippage")
    LIMIT_ORDER_BUFFER_PCT: float = Field(default=0.1, ge=0.0, le=5.0, description="Buffer percentage for limit orders (0.1 = 0.1%)")
    LIMIT_ORDER_TIMEOUT_SECONDS: int = Field(default=60, ge=10, le=300, description="Timeout before canceling unfilled limit order")
    ...
    def get_tier_allocations(self) -> dict[str, float]:
        ...
        if self.USE_OPTIMIZED_ALLOCATION:
            ...
            return {
                tier: amount * scale_factor
                for tier, amount in OPTIMIZED_AMOUNTS.items()
            }
        else:
