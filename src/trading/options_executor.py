...
class OptionsExecutor:
    ...
    def _check_daily_ic_limit(self) -> tuple[bool, int]:
        ...
        Reads system_state.json for today's IC open count.
        Returns (can_open, opens_today).
        """
        import json
    ...
    def _get_ml_guided_params(self) -> tuple[float, int, float] | None:
        """Get GRPO-optimized trade parameters if model is sufficiently trained.

        Returns (delta, dte, exit_pct) or None if model isn't ready.
        Safety: Only uses ML params when trained on >= 30 trades with confidence >= 0.6.
        Parameters are clamped to safe bounds from trading_thresholds.py.
        """
        import json
        ...
        try:
            ...
            logger.info(
                f"GRPO: using ML params delta={delta:.3f}, DTE={dte}, exit={exit_pct:.0%} (confidence={params.confidence:.2f})"
            )
    ...
    def _check_rag_lessons(self, ticker: str) -> list[str]:
        """Query RAG for relevant iron condor lessons that might affect this trade.

        Returns list of warning strings. Empty list = proceed normally.
        Critical lessons (severity >= 4) will be returned as warnings.
        """
        warnings = []
        ...
        try:
            ...
            pass  # RAG not available, proceed without
        except Exception as e:
