...
class FeedbackLearner:
    ...
    def create_lesson_from_negative_feedback(
        ...
        context_summary = entry.context[:200] if len(entry.context) > 200 else entry.context
...
"""
    ...
    def reinforce_positive_pattern(
        self, fb_id: str, entry: FeedbackEntry
    ) -> LearningOutcome:
        """
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "satisfaction_rate": (positive / len(all_feedback) * 100)
            if all_feedback
            else 0,
        }
...
def run_autonomous_learning() -> dict:
    ...
    summary = {
        ...
        "lessons_created": sum(
            1 for o in outcomes if o.action_taken == "lesson_created"
        ),
        "patterns_reinforced": sum(
            1 for o in outcomes if o.action_taken == "pattern_reinforced"
        ),
        "errors": sum(1 for o in outcomes if o.action_taken == "error"),
        "outcomes": [
            {"id": o.feedback_id, "action": o.action_taken, "details": o.details}
            for o in outcomes
        ],
