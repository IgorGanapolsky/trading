    ...
    def _init_subsystems(self) -> None:
        ...
        if self.enable_rag:
            try:
                ...
                # Always use reranking for pre-trade gate (critical decision point)
                self.rag_system = LessonsLearnedRAG(use_rerank=True)
                logger.info(
                    "RAG system initialized with Cohere Rerank for semantic anomaly detection"
                )
