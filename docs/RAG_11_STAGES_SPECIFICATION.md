...
### 2. Document Parsing (`messy_document_parser` + `LessonsLearnedRAG`)

- **Why does it exist?**: (A) Multi-format extraction for messy docs (PDF/HTML/tables/scanned gate) via `src/research/messy_document_parser.py` cascade Docling→pdfplumber→pypdf / HTML cleaner / quality gate. (B) Structured metadata (title, severity, tags) from lesson markdown into memory objects.
- **What can go wrong?**: Empty PDF extract indexed as knowledge; scanned PDFs without OCR; missing `.[documents]` deps; malformed lesson frontmatter.
- **How to measure working status**: `tests/test_messy_document_parser.py` green; quality gate rejects empty/garbage; 100% non-null title/severity on lessons (`test_lessons_learned_rag_smoke.py`). Spec: `docs/INGESTION_PARSING_APLUS.md`.
